...
def addToDatabase(keyword, description):
    db_config = {                                                                                                                                                                                                                 
     'host': '127.0.0.1',                                                                                                                                                                                                             
     'port': 3307,                                                                                                                                                                                                                    
     'user': 'root',                                                                                                                                                                                                                  
    ...
    try:                                                                                                                                                                                                                          
      ...
      query = "INSERT INTO keyword_abilities (keyword, description) VALUES (%s, %s) ON DUPLICATE KEY UPDATE description = %s;"
      cursor.execute(query, (keyword, description, description))                                                                                                                                                                 
      cnx.commit()
...
def getKeyDescription(keyword):
    ...
    url = "https://mtg.fandom.com/api.php?format=json&action=query&prop=revisions&rvprop=content&explaintext&redirects=1&titles="+keyword
    ...
    if response.status_code == 200:
        try:
          ...
          data = None
          print("INFO : Wiki not found for keyword "+keyword+", this keyword will be ignored")
...
def main():
        ...
        for keyword in keywords:
          ...
          if description is not None:
             ...
             addToDatabase(keyword, description)
